...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Для каждого Стр Из ДанныеОперации Цикл
		Если ЗначениеЗаполнено(Стр.Статья) Тогда
			Если Стр.НомерСтроки = 1 Тогда
				ОписаниеОперации = Стр.Статья.Наименование;
			Иначе
				ОписаниеОперации = ОписаниеОперации + ", " + Стр.Статья.Наименование;
			КонецЕсли; 
		Иначе
			ОписаниеОперации = ТипОперации;		
		КонецЕсли; 
	КонецЦикла; 
	ОписаниеОперации = ОписаниеОперации;
	СуммаДокумента = ДанныеОперации.Итог("Сумма");
