...
Процедура ОбновитьПоказателиНаФорме()
	...
	ИтогоДоход = Финансы.СуммаДоходовКРаспределению(КонецМесяца(ТекПериод));
	
	ВсегоРаспределено = Финансы.ВсегоРаспределено(, ТекПериод);
	ДоходовКРаспределению = ИтогоДоход - ВсегоРаспределено;
	...
	ОбщийБюджетНаДень = Окр(ИтогоДоход / ДнейДоКонцаМесяца, 2);
